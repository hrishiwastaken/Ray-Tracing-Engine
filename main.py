...
class Ray:
    def __init__(self, origin, direction, mirrors, bounces=10, color="orange"):
        self.origin = np.array(origin, dtype=float)
...
def on_click(event):
    ...
    # Assigning Right Click for placing mirror or lens
        ...
        # All mirror placements ask for radius
        else:
            facing = -1 if (event.key and "shift" in event.key.lower()) else 1
